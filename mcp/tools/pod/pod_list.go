...
func ListPod() mcp.Tool {
	return mcp.NewTool(
		"list_k8s_pod",
		mcp.WithDescription("获取Pod列表 (类似命令 kubectl get pods -n ns)。返回结果为分页格式，包含 items（当前页数据）、total（总数）、page（当前页码）、pageSize（每页大小）、totalPages（总页数）。如需获取更多数据，请增大 page 参数值。/ List pods with pagination. Response includes items, total, page, pageSize, totalPages."),
