...
func RunMCPServer(name, version string, port int) {
	...
	// 创建 SSE 服务器
	sseServer := createSSEServerWithMux(s)
...
func RunMCPServerWithOption(cfg *ServerConfig) {
	...
	if cfg.Mode == ServerModeStdio {
		...
		// 创建 SSE 服务器
		sseServer := createSSEServerWithMux(s, cfg.SSEOption...)
...
func createSSEServerWithMux(s *server.MCPServer, opts ...server.SSEOption) *server.SSEServer {
	mux := http.NewServeMux()

	// 注入自定义的 HTTP Server 给 SSEServer 以接管路由
	customOpts := append([]server.SSEOption{}, opts...)
	customOpts = append(customOpts, server.WithHTTPServer(&http.Server{
		Handler: mux,
	}))

	sseServer := server.NewSSEServer(s, customOpts...)
	streamableHTTPServer := server.NewStreamableHTTPServer(s)

	// 综合处理逻辑：支持 Streamable HTTP /mcp 以及 SSE /sse, /message 等路径
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		klog.Infof("Incoming request: %s %s from %s, headers: %v", r.Method, r.URL.Path, r.RemoteAddr, r.Header)
		...
		r.URL.Path = path // 统一路径格式，确保后续匹配正确

		// 兼容 Java mcp-core 0.17+ Streamable HTTP transport.
		if path == "/mcp" {
			klog.Infof("Handling %s /mcp (Streamable HTTP)", r.Method)
			streamableHTTPServer.ServeHTTP(w, r)
			return
		}

		// 处理标准路径 (/sse, /message) 或其他路径
